...
int xs=1;
try{
    String upl=(String)request.getAttribute("paylog");
    payja=wx.getJSONArrayfromJSONObject(upl, "paylog");
     xs=0;
}catch(Exception e){
	 xs=1;
}
...
for(int i=0;i<payja.size();i++){
	...
	String url="loginfo.jsp?pname="+pname+"&date="+payjob.getString("date")+"&mnum="+payjob.getString("mnum")+"&text="+text+"&time="+payjob.getString("time")+"&userlist="+payjob.getString("userlist")+"&no="+payjob.getString("no")+"&logno="+payjob.getString("logno")+"&us=1"+"&userid="+userid;
%>
